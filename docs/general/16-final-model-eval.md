...
## Why there is an NA-Plantae stage-2 SFT at all
...
sit at **0 / 100** on a ~10-species NA-Plantae probe set, while scoring
...
small NA-Plantae dataset** covering roughly 10 common species and ran a
...
"general plant ID") in exchange for jumping NA-Plantae from 0 % to 34 %
...
| Model | plant_300 | mmlu_50 | aime_20 | llava_40 | refusal_20 | NA-Plantae_100 | composite_5d |
|---|---|---|---|---|---|---|---|
| **r8-a8-nokl + NA-Plantae stage-2 step 11k** *(shipped)* | 31.0% | **50.0%** | **15.0%** | 12.0% | 100% | **34.0%** | 0.381 |
...
- `composite_5d` = unweighted mean of (plant, mmlu, aime, llava, refusal).
  NA-Plantae is reported separately because it lives on a different
...
**+34 pp on NA-Plantae** and **+10 pp on MMLU** vs that candidate.
...
### 1a. r8-a8-nokl + NA-Plantae stage-2 *(step 11000)* — shipped
...
small curated NA-Plantae set (~10 species), brief schedule, same r/α/KL.
...
The probe is intentionally easy: roughly 10 common NA Plantae, 100 in-the-wild
...
| **r8-a8-nokl + NA-Plantae stage-2 step 11k** *(shipped)* | **34 / 100** | **34.0 %** |

Three stage-1 candidates score zero — i.e. the PlantNet-300K-based
data mix does not transfer to these NA-Plantae species at all, even
when stage-1 plant_300 sits at 42 %. The stage-2 adapter is what
actually unlocks the domain.

### 2b. Stage-2 per-species pattern

A larger held-out pass over the curated NA-Plantae set shows the same
...
| Domain | Stage-1 only (1c, step 13k) | + NA-Plantae stage-2 (1a, step 11k) | Δ |
|---|---|---|---|
...
| NA-Plantae_100 | 0 % | 34.0 % | **+34.0 pp** |

Two effects worth calling out:

- **plant_300 drops 11.7 pp** as the model reallocates capacity from
  generic PlantNet-style identification toward NA-Plantae-specific
...
Per-run eval JSON artifacts and the NA-Plantae probe set live in
